...
## Step 4 — Catalog 설정 + Flink SQL Job 제출 (SSB REST API)

> **제출 경로:** Cloudera **SQL Stream Builder (SSB)** REST API — Web UI와 동일한 실행 경로입니다.  
> `./flink/run_*.sh` → `scripts/ssb_submit_sql.py`
...
또는 SSB Web UI → Jobs, Cloudera Manager → Flink → **Web UI**
...
| SSB connection refused | SSB 미기동 / URL 오류 | CM에서 SSB 확인, `.env` `SSB_API_BASE` 수정 |
| SSB 401/403 | Kerberos 만료 / project | `kinit` 재실행, `SSB_PROJECT_ID` 확인 |
| CREATE TABLE 실패 | `connector=iceberg` in iceberg catalog | connector DDL은 `default_catalog`에 생성 |
| INSERT parse 오류 | `TABLE (SELECT ...)` in TUMBLE | `CREATE TEMPORARY VIEW` 후 `TUMBLE(TABLE view_name, ...)` |
...
3. **Step 4 F-LTAS만** — `./flink/run_ltas_5min.sh` (SSB)
4. **Step 5** — 5분 후 결과 확인
5. **Step 6** — 데이터 추가 후 결과 변화 관찰
